$(window).load(function () {
	...
	function check_button(){
		...
		if(button==1){
			...
			$("#gallery-header-center-left-title").html('所有项目:');
		}
		if(button==2){
			$("#filter-studio").addClass(button_class);
			$("#gallery-header-center-left-title").html('游戏类:');
		}
		if(button==3){
			$("#filter-landscape").addClass(button_class);
			$("#gallery-header-center-left-title").html('网站:');
		}	
		if(button==4){
			$("#filter-girls").addClass(button_class);
			$("#gallery-header-center-left-title").html('美女图片:');
		}	
	}
	...
	$("#filter-girls").click(function() {  $container.isotope({ filter: '.girls' }); button = 4; check_button();  });
